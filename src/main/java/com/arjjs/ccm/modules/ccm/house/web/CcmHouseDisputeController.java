...
public class CcmHouseDisputeController extends BaseController {
	...
	public String list(CcmHouseDispute ccmHouseDispute, HttpServletRequest request, HttpServletResponse response,
		...
		if (StringUtils.isBlank(tableType)) {
			...
			String isShow = "NO";
			CcmHouseSetting setting = sysConfigService.getTimeInterval("ccmHouseDispute");
			if(StringUtils.isNotEmpty(setting.getTimeInterval()) && "1".equals(setting.getSendInfo())){
				...
				if(page.getList() != null && page.getList().size()>0){
					for(int i=0 ; i<page.getList().size() ; i++){
						CcmHouseDispute temp = page.getList().get(i);
			...
			for (CcmHouseDispute HouseDispute : list) {
				try {
					...
					if(list1.isEmpty()){
						...
						if(StringUtils.isEmpty(HouseDispute.getSex())){
							String sCardNum = HouseDispute.getIdent().substring(16, 17);
							if (Integer.parseInt(sCardNum) % 2 != 0) {
								HouseDispute.setSex("0");
							} else {
								HouseDispute.setSex("1");
							}
						}else{
							ccmPeople.setSex(HouseDispute.getSex());
						}
						if(StringUtils.isNotEmpty(HouseDispute.getTelephone())){
							//将导入时电话获取包含“.”或“E10”，的转成正确的电话格式
							if(HouseDispute.getTelephone().contains(".") || HouseDispute.getTelephone().contains("E")) {
								double bd = Double.valueOf(HouseDispute.getTelephone());
						...
						String year = HouseDispute.getIdent().substring(6, 10);
						String month = HouseDispute.getIdent().substring(10, 12);
						String day = HouseDispute.getIdent().substring(12, 14);
