...
public class CcmHouseDrugsController extends BaseController {
	...
	public String list(@Param("tableType")String tableType,CcmHouseDrugs ccmHouseDrugs, HttpServletRequest request, HttpServletResponse response,
		...
		if(StringUtils.isBlank(tableType)) {
			...
			String isShow = "NO";
			CcmHouseSetting setting = sysConfigService.getTimeInterval("ccmHouseDrugs");
			if(StringUtils.isNotEmpty(setting.getTimeInterval()) && "1".equals(setting.getSendInfo())){
				...
				if(page.getList() != null && page.getList().size()>0){
					for(int i=0 ; i<page.getList().size() ; i++){
						CcmHouseDrugs temp = page.getList().get(i);
			...
			for (CcmHouseDrugs HouseDrugs : list) {
				try {
					...
					if(list1.isEmpty()){
						...
						if(StringUtils.isEmpty(HouseDrugs.getSex())){
							String sCardNum = HouseDrugs.getIdent().substring(16, 17);
						...
						if(StringUtils.isNotEmpty(HouseDrugs.getTelephone())){
							//将导入时电话获取包含“.”或“E10”，的转成正确的电话格式
							if(HouseDrugs.getTelephone().contains(".") || HouseDrugs.getTelephone().contains("E")) {
								double bd = Double.valueOf(HouseDrugs.getTelephone());
