...
public class CcmHouseDeliberatelyIllegalController extends BaseController {
	...
	public String list(CcmHouseDeliberatelyIllegal ccmHouseDeliberatelyIllegal, HttpServletRequest request, HttpServletResponse response,
		...
		if(StringUtils.isBlank(tableType)){
			...
			String isShow = "NO";
			CcmHouseSetting setting = sysConfigService.getTimeInterval("ccmHouseDeliberatelyIllegal");
			if(StringUtils.isNotEmpty(setting.getTimeInterval()) && "1".equals(setting.getSendInfo())){
				...
				if(page.getList() != null && page.getList().size()>0){
					for(int i=0 ; i<page.getList().size() ; i++){
						CcmHouseDeliberatelyIllegal temp = page.getList().get(i);
			...
			for (CcmHouseDeliberatelyIllegal HouseDeliberatelyIllegal : list) {
				try {
					...
					if(list1.isEmpty()){
						...
						String year = HouseDeliberatelyIllegal.getIdent().substring(6, 10);
						String month = HouseDeliberatelyIllegal.getIdent().substring(10, 12);
						String day = HouseDeliberatelyIllegal.getIdent().substring(12, 14);
