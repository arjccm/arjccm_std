...
$(function(){
    ...
    $('#btnSubmitAreaImport').click(function(){
        currPage=1;
        QueryAreaImport();
    });
...
//监控
function QueryAreaImport(){
    debugger;
    var name=$('#name').val();
    var areaImport=$('#areaImportId').val();
    if(Map.overlayGISDialog){
        Map.overlayGISDialog.setPosition(undefined);
    }
    Map.clearOverlays();
    $.getJSON(ctx+'/sys/map/queryAreaImportMap?name='+encodeURIComponent(name)+'&area.id='+areaImport+'&pageNo='+currPage+'&pageSize='+pageSize,function(data){
        if(data==null||data==""||data==undefined){
            $.jBox.tip('暂无数据！');
            $('#pageNum').html('0');
            $('#datalist').html('');
        ...
        for(var i in featuresData){
            ...
            html+='<div class="col-row" data-id="'+id+'">'+data.features[i].properties.info['所属区域']+'</div>';
            html+='</div>';
            html+='</div>';
            html+='</li>';
        }
...
function playVideo(videoId, cameraType) {
    ...
    if(!isIE && !isIE11){
        var ccmDeviceParaml = '';
        $.ajaxSettings.async = false;
        $.getJSON('/arjccm/app/rest/video/getDevice', {"id": id}, function (data) {
            ccmDeviceParaml = data.result.param;
        });
        var host = window.location.host;
        var url = 'alert://' + host + '/videoPlay/ccmDeviceFormIE.html?param='+ccmDeviceParaml;
