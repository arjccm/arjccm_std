...
$(function() {
	...
	function showEchart() {
		...
		$.post(context + "/report/ccmPeopleStat/personStat", {
            ...
            lowLevelNew = data["所有下级区域本月未落户新增人数"];
            lowLevelAll = data["所有下级区域本月未落户总数"];
			// 图表1 数据
			...
			// 生成图表
			$.myChartPeopleAmountElse(title1, ajaxDataX1, seriesDate1, myChart1,"未落户人数",40,areaToContent);
			$.myChartPeople(title2, ajaxDataX2, seriesDate2, myChart2,"未落户人数变化趋势",0,optionToContent);
			// 生成 表单
    ...
    myChart1.on('click', function(params) {
        ...
        for (var i = 0; i < lowLevelAll.length ; i++) {
            if(typeof(id) == "undefined"){
                if((params.value == lowLevelAll[i]["value"]  || params.value == lowLevelNew[i]["value"]) && params.name == lowLevelAll[i]["type"]){
                    parentid = lowLevelAll[i]["value1"];
                    id = lowLevelAll[i]["value1"];
                }
            }else{
                if((params.value == lowLevelAll[i]["value"]  || params.value == lowLevelNew[i]["value"]) && params.name == lowLevelAll[i]["type"] && lowLevelAll[i]["value2"] == id ){
                    parentid = lowLevelAll[i]["value1"];
                    id = lowLevelAll[i]["value1"];
                }
            }
