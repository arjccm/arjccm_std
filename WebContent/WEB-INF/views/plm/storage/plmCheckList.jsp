...
<div class="back-list clearfix">
	<ul class="nav nav-tabs">
		<li class="active"><a class="nav-head" href="${ctx}/storage/plmCheck/">盘点单列表</a></li>
		<shiro:hasPermission name="storage:plmCheck:edit">
			<li><a style="text-align:center" href="${ctx}/storage/plmCheck/form">盘点单添加</a></li>
		</shiro:hasPermission>
	</ul>
                        ...
                        <c:when test="${plmCheck.status == '1'}">
                            <shiro:hasPermission name="storage:plmCheck:edit">
                                <td style="height: 50px"><a
                                        href="${ctx}/storage/plmCheck/form?id=${plmCheck.id}" class="btnList"><i
                                        title="修改" class="iconfont icon-caozuotubiao-xiugai"></i></a> <a
                                        href="${ctx}/storage/plmCheck/delete?id=${plmCheck.id}"
                                        onclick="return confirmx('确认要删除该盘点单吗？', this.href)" class="btnList"><i
                                        title="删除" class="iconfont icon-caozuotubiao-shanchu"></i></a></td>
                            </shiro:hasPermission>
                        </c:when>
                        <c:otherwise>
                            <td style="height: 50px"><a
                                    href="${ctx}/storage/plmCheck/form?id=${plmCheck.id}" class="btnList"><i title="查看"
                                                                                                             class="iconfont icon-caozuotubiao-xiangqing"></i></a>&nbsp;
                                <c:if test="${plmCheck.status != '1'}">
                                    <a title="check" id="${plmCheck.id}" class="btnList"><i title="盘点"
                        ...
                        <div class="thumbnail">
                            ...
                            <div class="caption row-fluid">
                                ...
                                <div class="spantext  " style="width: 63%; margin-left: 7%">
                                    ...
                                    <p
                                            title="${fns:getDictLabel(plmCheck.status, 'check_status', '不限')}">
                                        盘点状态:${fns:getDictLabel(plmCheck.status, 'check_status', '不限')}</p>
                                    <p
                            ...
                            <div class="footbtn" style="text-align: right;">
                                <c:choose>
                                    <c:when test="${plmCheck.status == '1'}">
                                        <shiro:hasPermission name="storage:plmCheck:edit">
                                            <a href="${ctx}/storage/plmCheck/form?id=${plmCheck.id}" class="btnList"><i
                                                    title="修改" class="iconfont icon-caozuotubiao-xiugai"></i></a>
                                            <a href="${ctx}/storage/plmCheck/delete?id=${plmCheck.id}"
                                               onclick="return confirmx('确认要删除该盘点单吗？', this.href)" class="btnList"><i
                                                    title="删除" class="iconfont icon-caozuotubiao-shanchu"></i></a>
                                        </shiro:hasPermission>
                                    </c:when>
                                    <c:otherwise>
                                        <a href="${ctx}/storage/plmCheck/form?id=${plmCheck.id}" class="btnList"><i
                                                title="查看" class="iconfont icon-caozuotubiao-xiangqing"></i></a>&nbsp;
                                        <c:if test="${plmCheck.status != '1'}">
                                            <a title="check" id="${plmCheck.id}" class="btnList"><i title="盘点"
                                                                                                    class="icon-search"></i></a>
                                        </c:if>
