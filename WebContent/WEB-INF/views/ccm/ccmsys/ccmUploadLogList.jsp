...
<div class="back-list clearfix">
    ...
    <form:form id="searchForm" modelAttribute="ccmUploadLog" action="${ctx}/ccmsys/ccmUploadLog/" method="post"
        ...
        <ul class="ul-form pull-left">
            ...
            <li class="first-line"><label>操作类型：</label>
                <form:select path="operateType" class="input-medium">
                    ...
                    <form:options items="${fns:getDictList('ccm_upload_log_operate_type')}" itemLabel="label"
            ...
            <li class="first-line"><label>上传状态：</label>
                <form:select path="uploadStatus" class="input-medium">
                    <form:option value="" label="全部"/>
                    <form:options items="${fns:getDictList('ccm_upload_log_upload_status')}" itemLabel="label"
                                  itemValue="value" htmlEscape="false"/>
                </form:select>
            </li>
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
        ...
        <tr>
            ...
            <th>更新时间</th>
            <th>备注信息</th>
            <shiro:hasPermission name="ccmsys:ccmUploadLog:edit">
                <th>操作</th>
            </shiro:hasPermission>
        </tr>
        </thead>
                ...
                <shiro:hasPermission name="ccmsys:ccmUploadLog:edit">
                    <td>
                        <c:if test="${ccmUploadLog.uploadStatus eq '3' }">
                            <a class="btnList" href="${ctx}/ccmsys/ccmUploadLog/form?id=${ccmUploadLog.id}"
                               title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>
                        </c:if>
                        <a class="btnList" href="${ctx}/ccmsys/ccmUploadLog/delete?id=${ccmUploadLog.id}"
                           onclick="return confirmx('确认要删除该待上传上级平台记录吗？', this.href)" title="删除"><i
