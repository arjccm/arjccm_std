...
<div class="back-list clearfix">
    <ul class="nav nav-tabs">
        ...
        <shiro:hasPermission name="patrol:ccmPatrolPlan:edit">
            <li><a href="${ctx}/patrol/ccmPatrolPlan/form">数据添加</a></li>
        </shiro:hasPermission>
    </ul>
    <form:form id="searchForm" modelAttribute="ccmPatrolPlan" action="${ctx}/patrol/ccmPatrolPlan/" method="post"
        ...
        <ul class="ul-form">
            ...
            <li><label>时间类型：</label>
                <form:select path="timeType" class="input-medium">
                    ...
                    <form:options items="${fns:getDictList('ccm_patrol_time_type')}" itemLabel="label" itemValue="value"
                                  htmlEscape="false"/>
                </form:select>
            </li>
            <li><label>启动状态：</label>
                <form:select path="status" class="input-medium">
                    <form:option value="" label="全部"/>
                    <form:options items="${fns:getDictList('ccm_patrol_status')}" itemLabel="label" itemValue="value"
                                  htmlEscape="false"/>
                </form:select>
            </li>
            <li><label>点位类型：</label>
                <form:select path="pointType" class="input-medium">
                    <form:option value="" label="全部"/>
                    <form:options items="${fns:getDictList('ccm_patrol_point_type')}" itemLabel="label"
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
        ...
        <c:forEach items="${page.list}" var="ccmPatrolPlan">
            <tr>
                ...
                <shiro:hasPermission name="patrol:ccmPatrolPlan:edit">
                    <td>
                        <a class="btnList" href="${ctx}/patrol/ccmPatrolPlan/form?id=${ccmPatrolPlan.id}" title="修改"><i
                                class="iconfont icon-caozuotubiao-xiugai"></i></a>
                        <a class="btnList" href="${ctx}/patrol/ccmPatrolPlan/delete?id=${ccmPatrolPlan.id}"
                           onclick="return confirmx('确认要删除该巡逻计划吗？', this.href)" title="删除"><i
                                class="iconfont icon-caozuotubiao-shanchu"></i></a>
                        <a class="btnList" href="${ctx}/patrol/ccmPatrolResult/startplan?planId=${ccmPatrolPlan.id}"
                           onclick="return confirmx('确认要开启该巡逻计划吗？', this.href)" title="开启计划"><i
                                class="icon-play"></i></a>
                    </td>
                </shiro:hasPermission>
            </tr>
