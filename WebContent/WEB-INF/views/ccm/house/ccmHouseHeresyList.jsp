...
<div class="context" content="${ctx}"></div>
	...
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/house/ccmHouseHeresy/import" method="post" enctype="multipart/form-data" class="form-search" style="padding-left: 20px; text-align: center;" onsubmit="return saveImport()"><br />
			<input id="uploadFile" name="file" type="file" style="width: 330px" onchange="changeFalse(this.value.length)" /><br /> <br />
			<input id="btnImportSubmit" class="btn btn-primary" type="submit" value="导  入 " />
		</form>
	</div>
	<ul class="nav nav-tabs">
		<shiro:hasPermission name="report:ccmPeopleStat:view"><li><a style="text-align:center" href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatHeresy">数据统计</a></li></shiro:hasPermission>
		<li class="active"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmHouseHeresy" onclick="HasSecret(this)">数据列表</a></li>
	</ul>
		...
		<c:forEach items="${page.list}" var="ccmHouseHeresy">
			<tr>
				...
				<td class="tc"><shiro:hasPermission name="house:ccmHouseHeresy:edit">
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/house/ccmHouseHeresy/form?id=${ccmHouseHeresy.id}', '修改', '1200px', '900px')" title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>
					<a class="btnList" href="${ctx}/house/ccmHouseHeresy/delete?id=${ccmHouseHeresy.id}&permissionKey=${permissionKey}" onclick="return confirmx('确认要删除该涉教人员吗？', this.href)" title="删除"><i class="iconfont icon-caozuotubiao-shanchu"></i></a>
					<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHouseHeresy.peopleId}')"  title="位置信息"><i class="iconfont icon-caozuotubiao-weizhixinxi "></i></a>
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHouseHeresy.peopleId}', '社交关系', '1000px', '700px')" title="社交关系"><i class="iconfont icon-caozuotubiao-shejiaoguanxi"></i></a>
				...
				<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseHeresy.id}&relevance_table=ccm_house_heresy', '添加记录', '800px', '660px')" title="添加记录"><i class="iconfont icon-caozuotubiao-tianjiachuli"></i></a>
