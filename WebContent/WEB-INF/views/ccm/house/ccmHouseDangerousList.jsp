...
<div class="back-list clearfix">
    ...
    <ul class="nav nav-tabs">
        <shiro:hasPermission name="report:ccmPeopleStat:view">
            ...
            <li><a style="text-align:center"
                   href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatDangerous">数据统计</a></li>
        </shiro:hasPermission>
        <li class="active"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmHouseDangerous"
                              onclick="HasSecret(this)">数据列表</a></li>
    </ul>
    <form:form id="searchForm" modelAttribute="ccmHouseDangerous" action="${ctx}/house/ccmHouseDangerous/" method="post"
    ...
    <ul class="ul-form pull-left">
        ...
        <li class="first-line"><label>危险品类别：</label>
            <form:select path="goodsType" class="input-medium">
                <form:option value="" label="全部"/>
                <form:options items="${fns:getDictList('ccm_house_dangerous_goods')}" itemLabel="label"
        ...
        <li class="first-line"><label>工作类别：</label>
            <form:select path="workType" class="input-medium">
                <form:option value="" label="全部"/>
                <form:options items="${fns:getDictList('ccm_house_dangerous_work')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select>
        </li>
    </ul>
    ...
    <div class="clearfix pull-right btn-box">
        ...
        <shiro:hasPermission name="house:ccmHouseDangerous:import">
        ...
        <shiro:hasPermission name="house:ccmHouseDangerous:export">
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
        ...
        <c:forEach items="${page.list}" var="ccmHouseDangerous">
            <tr>
                ...
                <td class="tc"><a href=""></a><a
                        onclick="parent.LayerDialog('${ctx}/house/ccmHouseDangerous/form?id=${ccmHouseDangerous.id}', '信息', '1200px', '900px')">${ccmHouseDangerous.name}</a>
                </td>
                ...
                <c:if test="${ccmHouseDangerous.atteType eq '01'}">
                    <td style='color:red'>${fns:getDictLabel(ccmHouseDangerous.atteType, 'ccm_conc_exte', '')}&nbsp;
                        &nbsp; <img src="${ctxStatic}/images/atteType_red.png"/></td>
                </c:if>
                <c:if test="${ccmHouseDangerous.atteType eq '02'}">
                    <td style='color:orange'>${fns:getDictLabel(ccmHouseDangerous.atteType, 'ccm_conc_exte', '')}&nbsp;
                ...
                <td class="tc"><shiro:hasPermission name="house:ccmHouseDangerous:edit">
                    ...
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHouseDangerous.peopleId}', '社交关系', '1000px', '700px')"
                    ...
                    <shiro:hasPermission name="log:ccmLogTail:edit">
                        <a class="btnList"
                           onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseDangerous.id}&relevance_table=ccm_house_dangerous', '记录信息', '800px', '660px')"
                           title="记录信息"><i class="iconfont icon-caozuotubiao-jiluxinxi"
                                           style="color: cornflowerblue;"></i></a>
                        <a class="btnList"
                           onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseDangerous.id}&relevance_table=ccm_house_dangerous', '添加记录', '800px', '660px')"
