		...
		<c:forEach items="${page.list}" var="ccmHouseDangerous">
			<tr>
				...
				<td class="tc"><shiro:hasPermission name="house:ccmHouseDangerous:edit">
	    			<a class="btnList" onclick="parent.LayerDialog('${ctx}/house/ccmHouseDangerous/form?id=${ccmHouseDangerous.id}', '修改', '1200px', '900px')" title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>
					<a class="btnList" href="${ctx}/house/ccmHouseDangerous/delete?id=${ccmHouseDangerous.id}permissionKey=${permissionKey}" onclick="return confirmx('确认要删除该危险品从业人员吗？', this.href)" title="删除"><i class="iconfont icon-caozuotubiao-shanchu"></i></a>
					<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHouseDangerous.peopleId}')"  title="位置信息"><i class="iconfont icon-caozuotubiao-weizhixinxi "></i></a>
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHouseDangerous.peopleId}', '社交关系', '1000px', '700px')" title="社交关系"><i class="iconfont icon-caozuotubiao-shejiaoguanxi"></i></a>
					...
					<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseDangerous.id}&relevance_table=ccm_house_dangerous', '添加记录', '800px', '660px')" title="添加记录"><i class="iconfont icon-caozuotubiao-tianjiachuli"></i></a>
