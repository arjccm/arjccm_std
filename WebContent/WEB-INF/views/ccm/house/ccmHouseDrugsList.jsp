...
<!-- 导入、导出模块 -->
	<div id="importBox" class="hide">
		<form id="importForm" action="${ctx}/house/ccmHouseDrugs/import" method="post" enctype="multipart/form-data" class="form-search" style="padding-left: 20px; text-align: center;" onsubmit="loading('正在导入，请稍等...');"><br />
			<input id="uploadFile" name="file" type="file" style="width: 330px" /><br /> <br /> <input id="btnImportSubmit" class="btn btn-primary" type="button" onclick="saveImport()" value="导  入 " />
		</form>
	</div>
			...
			<c:forEach items="${page.list}" var="ccmHouseDrugs">
				<tr>
					...
					<td class="tc"><shiro:hasPermission name="house:ccmHouseDrugs:edit">
							<a class="btnList" onclick="parent.LayerDialog('${ctx}/house/ccmHouseDrugs/form?id=${ccmHouseDrugs.id}', '修改', '1200px', '900px')" title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>
							<a class="btnList" href="${ctx}/house/ccmHouseDrugs/delete?id=${ccmHouseDrugs.id}&permissionKey=${permissionKey}" onclick="return confirmx('确认要删除该吸毒人口吗？', this.href)" title="删除"><i class="iconfont icon-caozuotubiao-shanchu"></i></a>
							<a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHouseDrugs.peopleId}')"  title="位置信息"><i class="iconfont icon-caozuotubiao-weizhixinxi "></i></a>
							<a class="btnList" onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHouseDrugs.peopleId}', '社交关系', '1000px', '700px')" title="社交关系"><i class="iconfont icon-caozuotubiao-shejiaoguanxi"></i></a>
							...
							<a class="btnList" onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseDrugs.id}&relevance_table=ccm_house_drugs', '添加记录', '800px', '660px')" title="添加记录"><i class="iconfont icon-caozuotubiao-tianjiachuli"></i></a>
