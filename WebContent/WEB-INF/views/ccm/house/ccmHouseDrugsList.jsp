...
<div class="back-list clearfix">
    ...
    <div id="importBox" class="hide">
        <form id="importForm" action="${ctx}/house/ccmHouseDrugs/import" method="post" enctype="multipart/form-data"
              class="form-search" style="padding-left: 20px; text-align: center;" onsubmit="loading('正在导入，请稍等...');">
            <br/>
            <input id="uploadFile" name="file" type="file" style="width: 330px" onchange="changeFalse(this.value.length)"/><br/> <br/> <input id="btnImportSubmit"
    ...
    <ul class="nav nav-tabs">
        <shiro:hasPermission name="report:ccmPeopleStat:view">
            <li><a style="text-align:center" href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatDrugs">数据统计</a>
            </li>
        </shiro:hasPermission>
        <li class="active"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmHouseDrugs"
                              onclick="HasSecret(this)">数据列表</a></li>
    </ul>
    <form:form id="searchForm" modelAttribute="ccmHouseDrugs" action="${ctx}/house/ccmHouseDrugs/" method="post"
    ...
    <ul class="ul-form pull-left">
        ...
        <li class="first-line"><label>吸毒原因：</label>
            <form:select path="drugCaus" class="input-medium">
                <form:option value="" label="全部"/>
                <form:options items="${fns:getDictList('sys_ccm_drugs_caus')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select></li>
    ...
    <div class="clearfix pull-right btn-box">
        ...
        <shiro:hasPermission name="house:ccmHouseDrugs:import">
        ...
        <shiro:hasPermission name="house:ccmHouseDrugs:export">
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
        ...
        <c:forEach items="${page.list}" var="ccmHouseDrugs">
            <tr>
                ...
                <td class="tc"><a
                        onclick="parent.LayerDialog('${ctx}/house/ccmHouseDrugs/form?id=${ccmHouseDrugs.id}', '信息', '1200px', '900px')">${ccmHouseDrugs.name}</a>
                </td>
                ...
                <c:if test="${ccmHouseDrugs.atteType eq '01'}">
                    <td style='color:red'>${fns:getDictLabel(ccmHouseDrugs.atteType, 'ccm_conc_exte', '')}&nbsp; &nbsp;
                        <img src="${ctxStatic}/images/atteType_red.png"/></td>
                </c:if>
                <c:if test="${ccmHouseDrugs.atteType eq '02'}">
                    <td style='color:orange'>${fns:getDictLabel(ccmHouseDrugs.atteType, 'ccm_conc_exte', '')}&nbsp;
                        &nbsp; <img src="${ctxStatic}/images/atteType_orange.png"/></td>
                </c:if>
                <c:if test="${ccmHouseDrugs.atteType eq '03'}">
                    <td>${fns:getDictLabel(ccmHouseDrugs.atteType, 'ccm_conc_exte', '')}&nbsp; &nbsp; <img
                ...
                <td class="tc"><shiro:hasPermission name="house:ccmHouseDrugs:edit">
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/house/ccmHouseDrugs/form?id=${ccmHouseDrugs.id}', '修改', '1200px', '900px')"
                    ...
                    <a class="btnList"
                       ...
                       onclick="return confirmx('确认要删除该吸毒人口吗？', this.href)" title="删除"><i
                            class="iconfont icon-caozuotubiao-shanchu"></i></a>
                    <a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHouseDrugs.peopleId}')"
