...
<div class="back-list clearfix">
    ...
    <form:form id="searchForm" modelAttribute="ccmHouseRectification"
               action="${ctx}/house/ccmHouseRectification/?tableType='ccmHouseRectification'" method="post"
    ...
    <ul class="ul-form pull-left">
        ...
        <li class="first-line"><label>矫正人员编号：</label> <form:input path="rectNum" htmlEscape="false" maxlength="16"
                                                                  class="input-medium"/></li>
        <li class="first-line"><label>矫正类別：</label>
            <form:select path="rectType" class="input-medium">
                <form:option value="" label="全部"/>
                <form:options items="${fns:getDictList('ccm_core_sort')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select></li>
        <li class="second-line"><label>案事件类別：</label>
            <form:select path="caseType" class="input-medium">
                <form:option value="" label="全部"/>
                <form:options items="${fns:getDictList('ccm_house_aids_cate')}" itemtd="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select>
        </li>
    ...
    <div class="clearfix pull-right btn-box">
        ...
        <a href="javascript:;"
           onclick="parent.parent.LayerDialog('${ctx}/house/ccmHouseEmphasis/listSet?type=ccmHouseRectification', '通知信息设置', '550px', '250px')"
    ...
    <table id="contentTable"
        ...
        <c:forEach items="${page.list}" var="ccmHouseRectification">
            <tr>
                ...
                <td>
                    <a onclick="parent.parent.LayerDialog('${ctx}/house/ccmHouseRectification/form?id=${ccmHouseRectification.id}', '信息', '1200px', '800px')">${ccmHouseRectification.name}</a>
                </td>
                ...
                <c:if test="${ccmHouseRectification.atteType eq '01'}">
                    <td style='color:red'>${fns:getDictLabel(ccmHouseRectification.atteType, 'ccm_conc_exte', '')}&nbsp;
                        &nbsp; <img src="${ctxStatic}/images/atteType_red.png"/></td>
                </c:if>
                <c:if test="${ccmHouseRectification.atteType eq '02'}">
                    <td style='color:orange'>${fns:getDictLabel(ccmHouseRectification.atteType, 'ccm_conc_exte', '')}&nbsp;
                        &nbsp; <img src="${ctxStatic}/images/atteType_orange.png"/></td>
                </c:if>
                <c:if test="${ccmHouseRectification.atteType eq '03'}">
                    <td>${fns:getDictLabel(ccmHouseRectification.atteType, 'ccm_conc_exte', '')}&nbsp; &nbsp; <img
                ...
                <td><shiro:hasPermission name="house:ccmHouseRectification:edit">
                    ...
                    <shiro:hasPermission name="log:ccmLogTail:edit">
                        <a class="btnList"
                           onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseRectification.id}&relevance_table=ccm_house_rectification', '走访记录', '1200px', '500px')"
                           title="走访记录"><i class="iconfont icon-caozuotubiao-zoufangjilu"></i></a>
                        <%-- <a class="btnList" onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseRectification.id}&relevance_table=ccm_house_rectification', '记录信息', '800px', '660px')" title="记录信息"><i class="iconfont icon-caozuotubiao-jiluxinxi" style="color: cornflowerblue;"></i></a> --%>
                        <a class="btnList"
                           onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseRectification.id}&relevance_table=ccm_house_rectification', '添加记录', '800px', '660px')"
