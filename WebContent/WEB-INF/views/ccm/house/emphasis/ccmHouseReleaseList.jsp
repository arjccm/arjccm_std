...
<div class="back-list clearfix" style="margin-left: 0">
    <input type="hidden" id="hasPermission" value="${fns:getUser().hasPermission}"/>
    <div class="context" content="${ctx}"></div>

    <form:form id="searchForm" modelAttribute="ccmHouseRelease"
               action="${ctx}/house/ccmHouseRelease/?tableType='ccmHouseRelease'" method="post"
    ...
    <ul class="ul-form pull-left">
        ...
        <li class="first-line"><label>原罪名：</label>
            <form:select path="origCha" class="input-medium">
                ...
                <form:options items="${fns:getDictList('ccm_chag_type')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select></li>

        <li class="first-line">
            <label>释放开始日期：</label>
            <input name="beginReleDate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
                   value="<fmt:formatDate value="${ccmHouseRelease.beginReleDate}" pattern="yyyy-MM-dd"/>"
                   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',isShowClear:false});"/></li>
        <li class="second-line"><label>释放结束日期：</label><input name="endReleDate" type="text" readonly="readonly"
                                                             maxlength="20" class="input-medium Wdate"
                                                             value="<fmt:formatDate value="${ccmHouseRelease.endReleDate}" pattern="yyyy-MM-dd"/>"
    ...
    <div class="clearfix pull-right btn-box">
        ...
        <a href="javascript:;"
           onclick="parent.parent.LayerDialog('${ctx}/house/ccmHouseEmphasis/listSet?type=ccmHouseRelease', '通知信息设置', '550px', '250px')"
    ...
    <table id="contentTable"
        ...
        <c:forEach items="${page.list}" var="ccmHouseRelease">
            <tr>
                ...
                <td>
                    <a onclick="parent.parent.LayerDialog('${ctx}/house/ccmHouseRelease/form?id=${ccmHouseRelease.id}', '信息', '1200px', '800px')">${ccmHouseRelease.name}</a>
                </td>
                ...
                <c:if test="${ccmHouseRelease.atteType eq '01'}">
                    <td style='color: red'>${fns:getDictLabel(ccmHouseRelease.atteType, 'ccm_conc_exte', '')}&nbsp;&nbsp;
                        <img src="${ctxStatic}/images/atteType_red.png"/></td>
                </c:if>
                <c:if test="${ccmHouseRelease.atteType eq '02'}">
                    <td style='color: orange'>${fns:getDictLabel(ccmHouseRelease.atteType, 'ccm_conc_exte', '')}&nbsp;&nbsp;
                        <img src="${ctxStatic}/images/atteType_orange.png"/></td>
                </c:if>
                <c:if test="${ccmHouseRelease.atteType eq '03'}">
                    <td>${fns:getDictLabel(ccmHouseRelease.atteType, 'ccm_conc_exte', '')}&nbsp;&nbsp; <img
                            src="${ctxStatic}/images/atteType_green.png"/></td>
                </c:if>
                <c:if test="${ccmHouseRelease.atteType eq '04'}">
                ...
                <td><shiro:hasPermission name="house:ccmHouseRelease:edit">
                        ...
                        <a class="btnList"
                           onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseRelease.id}&relevance_table=ccm_house_release', '走访记录', '1200px', '500px')"
                           title="走访记录"><i class="iconfont icon-caozuotubiao-zoufangjilu"></i></a>
                        <%-- <a class="btnList" onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseRelease.id}&relevance_table=ccm_house_release', '记录信息', '800px', '660px')" title="记录信息"><i class="iconfont icon-caozuotubiao-jiluxinxi" style="color: cornflowerblue;"></i></a> --%>
                </shiro:hasPermission> <shiro:hasPermission name="log:ccmLogTail:edit">
                    <a class="btnList"
                           onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseRelease.id}&relevance_table=ccm_house_release', '添加记录', '800px', '660px')"
