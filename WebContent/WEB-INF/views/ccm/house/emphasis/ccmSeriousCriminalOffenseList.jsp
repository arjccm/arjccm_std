...
<div class="back-list clearfix">
    ...
    <form:form id="searchForm" modelAttribute="ccmSeriousCriminalOffense"
               action="${ctx}/house/ccmSeriousCriminalOffense/?tableType='ccmSeriousCriminalOffense'" method="post"
    ...
    <ul class="ul-form pull-left">
        ...
        <li class="first-line">
            ...
            <form:select path="dangerLevel" class="input-medium">
                <form:option value="" label="全部"/>
                <form:options items="${fns:getDictList('danger_level_dict')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select>
        </li>
        <!-- 导入、导出模块 -->
        ...
        <li class="first-line">
            ...
            <form:select path="suspicionType" class="input-medium">
                <form:option value="" label="全部"/>
                <form:options items="${fns:getDictList('criminal_type')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select>
        </li>
        <li class="first-line">
    ...
    <div class="clearfix pull-right btn-box">
        ...
        <a href="javascript:;"
           onclick="parent.parent.LayerDialog('${ctx}/house/ccmHouseEmphasis/listSet?type=ccmSeriousCriminalOffense', '通知信息设置', '550px', '250px')"
        ...
        <c:forEach items="${page.list}" var="ccmSeriousCriminalOffense">
            <tr>
                ...
                <c:if test="${ccmSeriousCriminalOffense.dangerLevel eq '01'}">
                    <td style='color:red'>${fns:getDictLabel(ccmSeriousCriminalOffense.dangerLevel, 'danger_level_dict', '')}&nbsp;
                        &nbsp; <img src="${ctxStatic}/images/atteType_red.png"/></td>
                </c:if>
                <c:if test="${ccmSeriousCriminalOffense.dangerLevel eq '02'}">
                    <td style='color:orange'>${fns:getDictLabel(ccmSeriousCriminalOffense.dangerLevel, 'danger_level_dict', '')}&nbsp;
                        &nbsp; <img src="${ctxStatic}/images/atteType_orange.png"/></td>
                </c:if>
                <c:if test="${ccmSeriousCriminalOffense.dangerLevel eq '03'}">
                    <td>${fns:getDictLabel(ccmSeriousCriminalOffense.dangerLevel, 'danger_level_dict', '')}&nbsp; &nbsp;
                ...
                <td>
                    <shiro:hasPermission name="house:ccmSeriousCriminalOffense:edit">
                        <a class="btnList"
                           onclick="parent.parent.LayerDialog('${ctx}/house/ccmSeriousCriminalOffense/form?id=${ccmSeriousCriminalOffense.id}&hide1=true&hide2=false', '详情', '1200px', '800px')"
                           title="详情"><i class="iconfont icon-caozuotubiao-xiangqing"></i></a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="log:ccmLogTail:view">
                        <a class="btnList"
                           onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmSeriousCriminalOffense.id}&relevance_table=ccm_serious_criminal_offense', '走访记录', '1200px', '500px')"
                           title="走访记录"><i class="iconfont icon-caozuotubiao-zoufangjilu"></i></a>
                        <%-- <a class="btnList" onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmSeriousCriminalOffense.id}&relevance_table=ccm_serious_criminal_offense', '记录信息', '800px', '660px')" title="记录信息"><i class="iconfont icon-caozuotubiao-jiluxinxi" style="color: cornflowerblue;"></i></a> --%>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="log:ccmLogTail:edit">
                        <a class="btnList"
                           onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmSeriousCriminalOffense.id}&relevance_table=ccm_serious_criminal_offense', '添加记录', '800px', '660px')"
