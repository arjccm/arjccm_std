...
<div class="back-list clearfix">
    ...
    <form:form id="searchForm" modelAttribute="ccmHouseAids"
               action="${ctx}/house/ccmHouseAids/?tableType='ccmHouseAids'" method="post"
    ...
    <ul class="ul-form pull-left">
        ...
        <li class="first-line"><label>收治情况：</label>
            <form:select path="treaCase" class="input-medium">
                <form:option value="" label="全部"/>
                <form:options items="${fns:getDictList('ccm_tred_case')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select></li>
    ...
    <div class="clearfix pull-right btn-box">
        ...
        <a href="javascript:;"
           onclick="parent.parent.LayerDialog('${ctx}/house/ccmHouseEmphasis/listSet?type=ccmHouseAids', '通知信息设置', '550px', '250px')"
        ...
        <c:forEach items="${page.list}" var="ccmHouseAids">
            <tr>
                ...
                <c:if test="${ccmHouseAids.atteType eq '01'}">
                    <td style='color:red'>${fns:getDictLabel(ccmHouseAids.atteType, 'ccm_conc_exte', '')}&nbsp; &nbsp;
                        <img src="${ctxStatic}/images/atteType_red.png"/></td>
                </c:if>
                <c:if test="${ccmHouseAids.atteType eq '02'}">
                    <td style='color:orange'>${fns:getDictLabel(ccmHouseAids.atteType, 'ccm_conc_exte', '')}&nbsp;
                        &nbsp; <img src="${ctxStatic}/images/atteType_orange.png"/></td>
                </c:if>
                <c:if test="${ccmHouseAids.atteType eq '03'}">
                    <td>${fns:getDictLabel(ccmHouseAids.atteType, 'ccm_conc_exte', '')}&nbsp; &nbsp; <img
                ...
                <td><shiro:hasPermission name="house:ccmHouseAids:edit">
                    ...
                    <shiro:hasPermission name="log:ccmLogTail:view">
                        <a class="btnList"
                           onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseAids.id}&relevance_table=ccm_house_aids', '走访记录', '1200px', '500px')"
                           title="走访记录"><i class="iconfont icon-caozuotubiao-zoufangjilu"></i></a>
                        <%-- <a class="btnList" onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseAids.id}&relevance_table=ccm_house_aids', '记录信息', '800px', '660px')" title="记录信息"><i class="iconfont icon-caozuotubiao-jiluxinxi" style="color: cornflowerblue;"></i></a> --%>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="log:ccmLogTail:edit">
                        <a class="btnList"
                           onclick="parent.parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseAids.id}&relevance_table=ccm_house_aids', '添加记录', '800px', '660px')"
