...
<div class="back-list clearfix">
    ...
    <ul class="nav nav-tabs">
        ...
        <shiro:hasPermission name="report:ccmPeopleStat:view">
            <li><a style="text-align:center"
                   href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatDispute">数据统计</a></li>
        </shiro:hasPermission>
        <li class="active"><a class="nav-head" href="javascript:;" data-href="${ctx}/house/ccmHouseDispute"
                              onclick="HasSecret(this)">数据列表</a></li>
    </ul>
    <form:form id="searchForm" modelAttribute="ccmHouseDispute" action="${ctx}/house/ccmHouseDispute/" method="post"
    ...
    <div class="clearfix pull-right btn-box">
        ...
        <shiro:hasPermission name="house:ccmHouseDispute:import">
        ...
        <shiro:hasPermission name="house:ccmHouseDispute:export">
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
        ...
        <c:forEach items="${page.list}" var="ccmHouseDispute">
            <tr>
                ...
                <td class="tc"><a
                        onclick="parent.LayerDialog('${ctx}/house/ccmHouseDispute/form?id=${ccmHouseDispute.id}', '信息', '1200px', '900px')">${ccmHouseDispute.name}</a>
                </td>
                ...
                <c:if test="${ccmHouseDispute.dangerLevel eq '01'}">
                    <td style='color:red'>${fns:getDictLabel(ccmHouseDispute.dangerLevel, 'ccm_conc_exte', '')}&nbsp;
                        &nbsp; <img src="${ctxStatic}/images/atteType_red.png"/></td>
                </c:if>
                <c:if test="${ccmHouseDispute.dangerLevel eq '02'}">
                    <td style='color:orange'>${fns:getDictLabel(ccmHouseDispute.dangerLevel, 'ccm_conc_exte', '')}&nbsp;
                ...
                <td class="tc"><shiro:hasPermission name="house:ccmHarmNationalSecurity:edit">
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/house/ccmHouseDispute/form?id=${ccmHouseDispute.id}', '修改', '1200px', '900px')"
                       title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>
                    <a class="btnList"
                       href="${ctx}/house/ccmHouseDispute/delete?id=${ccmHouseDispute.id}&permissionKey=${permissionKey}"
                       onclick="return confirmx('确认要删除闹事行凶报复嫌疑人口吗？', this.href)" title="删除"><i
                            class="iconfont icon-caozuotubiao-shanchu"></i></a>
                    <a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHouseDispute.peopleId}')"
                       title="位置信息"><i class="iconfont icon-caozuotubiao-weizhixinxi "></i></a>
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHouseDispute.peopleId}', '社交关系', '1000px', '700px')"
