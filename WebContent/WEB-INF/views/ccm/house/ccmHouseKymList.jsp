...
<div class="back-list clearfix">
    ...
    <ul class="nav nav-tabs">
        <shiro:hasPermission name="report:ccmPeopleStat:view">
            <li style="width: 112px;"><a href="${ctx}/report/ccmPeopleStat/statisticsPage?title=ccmPeopleStatTeenager"
                                         style="text-align: center">数据统计</a></li>
        </shiro:hasPermission>
    ...
    <form:form id="searchForm" modelAttribute="ccmHouseKym" action="${ctx}/house/ccmHouseKym/" method="post"
               class="breadcrumb form-search clearfix">
    <input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
    <input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
    <ul class="ul-form pull-left">
        <li class="first-line"><label class="title-text">姓名：</label> <form:input path="name" htmlEscape="false"
        ...
        <li class="first-line"><label class="title-text">人员类型：</label>
            <form:select path="manType" class="input-medium">
                <form:option value="" label=""/>
                <form:options items="${fns:getDictList('ccm_delp_type')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select></li>

            <%--			<li class="clearfix"></li>--%>
        <li class="first-line"><label class="title-text">家庭情况：</label> <form:select path="famiStat"
                                                                                    class="input-medium">
            <form:option value="" label="全部"/>
            <form:options items="${fns:getDictList('ccm_famy_stat')}" itemLabel="label" itemValue="value"
                          htmlEscape="false"/>
        </form:select>
        </li>
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
        ...
        <c:forEach items="${page.list}" var="ccmHouseKym">
            <tr>
                ...
                <td>
                    <a onclick="parent.LayerDialog('${ctx}/house/ccmHouseKym/form?id=${ccmHouseKym.id}', '信息', '1200px', '900px')">${ccmHouseKym.name} </a>
                </td>
                ...
                <c:if test="${ccmHouseKym.atteType eq '01'}">
                    <td style='color:red'>${fns:getDictLabel(ccmHouseKym.atteType, 'ccm_conc_exte', '')}&nbsp; &nbsp;
                        <img src="${ctxStatic}/images/atteType_red.png"/></td>
                </c:if>
                <c:if test="${ccmHouseKym.atteType eq '02'}">
                    <td style='color:orange'>${fns:getDictLabel(ccmHouseKym.atteType, 'ccm_conc_exte', '')}&nbsp; &nbsp;
                ...
                <td><shiro:hasPermission name="house:ccmHouseKym:edit">
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/house/ccmHouseKym/form?id=${ccmHouseKym.id}', '信息', '1200px', '900px')"
                       title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>
                    <a class="btnList" href="${ctx}/house/ccmHouseKym/delete?id=${ccmHouseKym.id}"
                       onclick="return confirmx('确认要删除该重点青少年吗？', this.href)" title="删除"><i
                            class="iconfont icon-caozuotubiao-shanchu"></i></a>
                    <a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmHouseKym.peopleId}')"
                       title="位置信息"><i class="iconfont icon-caozuotubiao-weizhixinxi "></i></a>
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/pop/ccmPeople/getSocialConnections?id=${ccmHouseKym.peopleId}', '社交关系', '1000px', '700px')"
                    ...
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmHouseKym.id}&relevance_table=ccm_house_kym', '记录信息', '800px', '660px')"
                       title="记录信息"><i class="iconfont icon-caozuotubiao-jiluxinxi" style="color: cornflowerblue;"></i></a>
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmHouseKym.id}&relevance_table=ccm_house_kym', '添加记录', '800px', '660px')"
