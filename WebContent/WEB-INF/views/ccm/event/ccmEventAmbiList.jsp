...
<div class="back-list clearfix">
    ...
    <ul class="nav nav-tabs">
        <shiro:hasPermission name="report:ccmEventAmbi:view">
        ...
        <shiro:hasPermission name="event:ccmEventAmbi:edit">
            <li><a style="text-align:center" href="${ctx}/event/ccmEventAmbi/form">矛盾纠纷添加</a></li>
        </shiro:hasPermission>
    </ul>
    <form:form id="searchForm" modelAttribute="ccmEventAmbi" action="${ctx}/event/ccmEventAmbi/" method="post"
    ...
    <ul class="ul-form pull-left">
        ...
        <li class="first-line"><label>开始时间：</label> <input
                ...
                onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false,maxDate:'#F{$dp.$D(\'endSendDate\')||\'%y-%M-%d\'}'});"/>
        </li>
        <li class="first-line"><label>结束时间：</label><input name="endSendDate" id="endSendDate" type="text"
                                                          readonly="readonly"
                                                          maxlength="20" class="input-medium Wdate"
        ...
        <li class="first-line"><label>所属网格：</label>
            <sys:treeselect id="area" name="area.id" value="${ccmEventAmbi.area.id}" labelName="area.name"
                            labelValue="${ccmEventAmbi.area.name}"
        ...
        <li class="first-line"><label>矛盾纠纷规模：</label>
            <form:select path="eventScale" class="input-medium">
                ...
                <form:options items="${fns:getDictList('ccm_event_scale')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select>
        </li>
        <li class="second-line"><label>矛盾纠纷类别：</label>
            <form:select path="eventType" class="input-medium">
                <form:option value="" label="全部"/>
                <form:options items="${fns:getDictList('ccm_event_sort')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select>
        </li>
            ...
            <tr>
                ...
                <td><shiro:hasPermission name="event:ccmEventAmbi:edit">
                    ...
                    <a class="btnList" href="${ctx}/event/ccmEventAmbi/delete?id=${ccmEventAmbi.id}"
                       onclick="return confirmx('确认要删除该矛盾纠纷排查化解吗？', this.href)" title="删除"><i
                            class="iconfont icon-caozuotubiao-shanchu"></i></a>
                    <a class="btnList" href="javascript:;" onclick="LocationOpen('${ccmEventAmbi.id}')" title="位置信息"><i
