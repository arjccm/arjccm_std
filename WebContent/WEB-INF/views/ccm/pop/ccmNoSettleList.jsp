...
<div class="context" content="${ctx}"></div>
 <!-- 导入、导出模块 -->
	...
	<ul class="nav nav-tabs">
		<li class="active"><a class="nav-head" href="${ctx}/pop/ccmWork/list/40">数据列表</a></li>
	</ul>
...
<%--			<li class="clearfix selectHidden hide"></li>--%>
	...
	<ul class="ul-form pull-left">
			...
			<li class="selectHidden hide"><label>出生结束日期：</label><input name="endBirthday" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate"
		...
		<c:forEach items="${page.list}" var="ccmPeople">
			<tr>
				...
				<td><%--<a href="${ctx}/pop/ccmWork/form/noSettle?id=${ccmPeople.id}">
					${ccmPeople.name}</a>--%>
					<a class="btnList"
					   onclick="parent.LayerDialog('${ctx}/pop/ccmWork/form/noSettle?id=${ccmPeople.id}', '详情', '1500px', '700px')"
					   >${ccmPeople.name}</a>
				...
				<td><shiro:hasPermission name="pop:ccmPermanent:edit">
	    				<%--<a class="btnList"  href="${ctx}/pop/ccmWork/form/noSettle?id=${ccmPeople.id}"  title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>--%>

					<a class="btnList"
					   onclick="parent.LayerDialog('${ctx}/pop/ccmWork/form/noSettle?id=${ccmPeople.id}', '编辑', '1500px', '700px')"
					   title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>

						<a class="btnList" href="${ctx}/pop/ccmWork/delete/noSettle?id=${ccmPeople.id}" onclick="return confirmx('确认要删除该实有人口吗？', this.href)"  title="删除"><i class="iconfont icon-caozuotubiao-shanchu"></i></a>
				   <a class="btnList"
