...
<div class="back-list clearfix">
    ...
    <ul class="nav nav-tabs">
        <li class="active"><a class="nav-head" href="${ctx}/pop/ccmWork/list/30">数据列表</a></li>
    </ul>
    <form:form id="searchForm" modelAttribute="ccmPeople" action="${ctx}/pop/ccmWork/list/30" method="post"
    ...
    <ul class="ul-form pull-left">
        ...
        <li class="selectHidden hide"><label>所属社区：</label>
            <sys:treeselect id="areaComId" name="areaComId.id" value="${ccmPeople.areaComId.id}"
                            ...
                            title="社区" url="/tree/ccmTree/treeDataArea?type=6" cssClass="input-medium" allowClear="true"
        ...
        <li class="selectHidden hide"><label>出生结束日期：</label> <input name="endBirthday" type="text" readonly="readonly"
        ...
        <c:forEach items="${page.list}" var="ccmPeople">
            <tr>
                ...
                <td><%--<a href="${ctx}/pop/ccmWork/form/overseas?id=${ccmPeople.id}">
                        ${ccmPeople.name}</a>--%>
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/pop/ccmWork/form/overseas?id=${ccmPeople.id}', '详情', '1500px', '700px')">${ccmPeople.name}</a>
                ...
                <td><shiro:hasPermission name="pop:ccmPermanent:edit">
                    <%--<a class="btnList"  href="${ctx}/pop/ccmWork/form/overseas?id=${ccmPeople.id}"  title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>--%>

                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/pop/ccmWork/form/overseas?id=${ccmPeople.id}', '编辑', '1500px', '700px')"
                       title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>

                    <a class="btnList" href="${ctx}/pop/ccmWork/delete/overseas?id=${ccmPeople.id}"
                       onclick="return confirmx('确认要删除该实有人口吗？', this.href)" title="删除"><i
                            class="iconfont icon-caozuotubiao-shanchu"></i></a>
                    <a class="btnList"
                       href="javascript:;" onclick="LocationOpen('${ccmPeople.id}')" title="位置信息"><i
                    ...
                    <shiro:hasPermission name="log:ccmLogTail:edit">
                        ...
                        <a class="btnList"
                           onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formProPermanent?relevance_id=${ccmPeople.id}&relevance_table=ccm_people', '添加记录', '800px', '660px')"
