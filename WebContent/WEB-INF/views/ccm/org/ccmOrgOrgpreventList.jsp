...
<ul class="back-list clearfix">
    <ul class="nav nav-tabs">
        <li class="active"><a class="nav-head" href="${ctx}/org/ccmOrgOrgprevent/">数据列表</a></li>
        <%-- <shiro:hasPermission name="org:ccmOrgOrgprevent:edit"><li><a href="${ctx}/org/ccmOrgOrgprevent/form">数据添加</a></li></shiro:hasPermission> --%>
    </ul>
    <form:form id="searchForm" modelAttribute="ccmOrgOrgprevent" action="${ctx}/org/ccmOrgOrgprevent/" method="post"
        ...
        <ul class="ul-form pull-left">
            ...
            <li class="first-line"><label>归属区域：</label>
                <sys:treeselect id="area" name="area.id" value="${ccmOrgOrgprevent.area.id}" labelName="area.name"
                                labelValue="${ccmOrgOrgprevent.area.name}"
            ...
            <li class="first-line"><label>组织类型：</label>
                <form:select path="comType" class="input-medium">
                    ...
                    <form:options items="${fns:getDictList('ccm_org_type')}" itemLabel="label" itemValue="value"
                                  htmlEscape="false"/>
                </form:select>
            </li>
            <li class="first-line"><label>组织层级：</label>
                <form:select path="comScale" class="input-medium">
                    <form:option value="" label="全部"/>
                    <form:options items="${fns:getDictList('ccm_ply_rat')}" itemLabel="label" itemValue="value"
                                  htmlEscape="false"/>
                </form:select>
            </li>
            ...
            <tr>
                ...
                <shiro:hasPermission name="org:ccmOrgOrgprevent:edit">
                    <td style="height: 50px">
                        ...
                        <a class="btnList" href="${ctx}/org/ccmOrgOrgprevent/delete?id=${ccmOrgOrgprevent.id}"
                           onclick="return confirmx('确认要删除该群防群治组织吗？', this.href)" title="删除"><i
