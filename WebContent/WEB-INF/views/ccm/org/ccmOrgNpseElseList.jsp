...
<ul class="back-list clearfix">
    <ul class="nav nav-tabs">
        <li class="active"><a class="nav-head" href="${ctx}/org/ccmOrgNpseElse/">数据列表</a></li>
        <%-- <shiro:hasPermission name="org:ccmOrgNpseElse:edit"><li><a href="${ctx}/org/ccmOrgNpseElse/form">其他重点添加</a></li></shiro:hasPermission> --%>
    </ul>
    <form:form id="searchForm" modelAttribute="ccmOrgNpse" action="${ctx}/org/ccmOrgNpseElse/" method="post"
        ...
        <ul class="ul-form pull-left">
            ...
            <li class="first-line"><label>所属网格：</label>
                <sys:treeselect id="area" name="area.id" value="${ccmOrgNpse.area.id}" labelName="area.name"
                                ...
                                title="区域" url="/tree/ccmTree/treeDataArea?type=7&areaid=" cssClass="input-medium"
                                allowClear="true" notAllowSelectParent="true"/>
            </li>
            <li class="first-line"><label>类别：</label>
                <form:select path="compType" class="input-medium">
                    <form:option value="" label="全部"/>
                    <form:options items="${fns:getDictList('ccm_buss_cate')}" itemLabel="label" itemValue="value"
                                  htmlEscape="false"/>
                </form:select>
            </li>
            <!--
            ...
            <li class="first-line"><label>安全隐患类型：</label>
                <form:select path="safeHazaType" class="input-medium">
                    <form:option value="" label="全部"/>
                    <form:options items="${fns:getDictList('ccm_pori_type')}" itemLabel="label" itemValue="value"
                                  htmlEscape="false"/>
                </form:select>
            </li>
            <li class="second-line"><label>风险级别：</label>
                <form:select path="riskRank" class="input-medium">
                    <form:option value="" label="全部"/>
                    <form:options items="${fns:getDictList('ccm_npse_risk_rank')}" itemLabel="label" itemValue="value"
                                  htmlEscape="false"/>
                </form:select>
            </li>
            ...
            <li class="second-line"><label>登记注册结束日期：</label><input name="endRegisterDate" type="text"
        ...
        <div class="clearfix pull-right btn-box">
            <shiro:hasPermission name="org:ccmOrgNpseElse:edit">
                <a onclick="parent.LayerDialog('${ctx}/org/ccmOrgNpseElse/form', '添加', '1100px', '600px')"
    ...
    <table id="contentTable" class="table table-striped table-bordered table-condensed table-gradient">
        ...
        <c:forEach items="${page.list}" var="ccmOrgNpse">
            <tr>
                ...
                <td>
                    <a onclick="parent.LayerDialog('${ctx}/org/ccmOrgNpseElse/form?id=${ccmOrgNpse.id}', '详情', '1100px', '800px')">${ccmOrgNpse.compName}</a>
                ...
                <td>
                    <shiro:hasPermission name="org:ccmOrgNpseElse:edit">
                        <a class="btnList"
                           onclick="parent.LayerDialog('${ctx}/org/ccmOrgNpseElse/form?id=${ccmOrgNpse.id}', '编辑', '1100px', '800px')"
                           title="修改"><i class="iconfont icon-caozuotubiao-xiugai"></i></a>
                        <a class="btnList" href="${ctx}/org/ccmOrgNpseElse/delete?id=${ccmOrgNpse.id}"
                           onclick="return confirmx('确认要删除该其他重点吗？', this.href)" title="删除"><i
                                class="iconfont icon-caozuotubiao-shanchu"></i></a>
                    </shiro:hasPermission>
                    <shiro:hasPermission name="log:ccmLogTail:edit">
                        <a class="btnList"
                           onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${ccmOrgNpse.id}&relevance_table=ccm_org_npse', '记录信息', '800px', '660px')"
                           title="记录信息"><i class="iconfont icon-caozuotubiao-jiluxinxi"
                                           style="color: cornflowerblue;"></i></a>
                        <a class="btnList"
                           onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formPro?relevance_id=${ccmOrgNpse.id}&relevance_table=ccm_org_npse', '添加记录', '800px', '660px')"
