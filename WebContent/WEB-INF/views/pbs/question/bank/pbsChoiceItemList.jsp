		...
		<tbody>
			<c:forEach items="${page.list}" var="pbsChoiceItem">
				<tr>
					...
					<shiro:hasPermission name="question:pbsChoiceItem:edit">
						<td><a
							href="${ctx}/question/pbsChoiceItem/form?id=${pbsChoiceItem.id}" title = "修改"><i class="icon iconfont icon-caozuotubiao-xiugai"></i></a>
							<a
							href="${ctx}/question/pbsChoiceItem/delete?id=${pbsChoiceItem.id}"
							onclick="return confirmx('确认要删除该选择题选项吗？', this.href)" title = "删除"><i class="icon iconfont icon-caozuotubiao-shanchu"></i></a></td>
					</shiro:hasPermission>
				</tr>
			</c:forEach>
		</tbody>
