		...
		<tbody>
			<c:forEach items="${page.list}" var="pbsProposalopt">
				<tr>
					...
					<shiro:hasPermission name="proposal:pbsProposalopt:edit">
						<td><a
							href="${ctx}/proposal/pbsProposalopt/form?id=${pbsProposalopt.id}" title = "修改"><i class="icon iconfont icon-caozuotubiao-xiugai"></i></a>
							<a
							href="${ctx}/proposal/pbsProposalopt/delete?id=${pbsProposalopt.id}"
							onclick="return confirmx('确认要删除该建议操作信息吗？', this.href)" title = "删除"><i class="icon iconfont icon-caozuotubiao-shanchu"></i></a></td>
					</shiro:hasPermission>
				</tr>
			</c:forEach>
		</tbody>
