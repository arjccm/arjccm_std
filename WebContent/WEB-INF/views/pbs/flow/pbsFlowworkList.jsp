		...
		<tbody>
			<c:forEach items="${page.list}" var="pbsFlowwork">
				<tr>
					...
					<shiro:hasPermission name="flow:pbsFlowwork:edit">
						<td><a
							href="${ctx}/flow/pbsFlowwork/form?id=${pbsFlowwork.id}" title = "修改"><i class="icon iconfont icon-caozuotubiao-xiugai"></i></a> <a
							href="${ctx}/flow/pbsFlowwork/delete?id=${pbsFlowwork.id}"
							onclick="return confirmx('确认要删除该运行工作流吗？', this.href)" title = "删除"><i class="icon iconfont icon-caozuotubiao-shanchu"></i></a></td>
					</shiro:hasPermission>
				</tr>
			</c:forEach>
		</tbody>
