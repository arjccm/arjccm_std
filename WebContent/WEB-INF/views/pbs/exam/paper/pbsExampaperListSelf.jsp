		...
		<tbody>
			<c:forEach items="${page.list}" var="pbsExampaper">
				<tr>
					...
					<shiro:hasPermission name="exam:pbsExampaper:edit">
						<td><a
							href="${ctx}/exam/pbsExampaper/formSelf?id=${pbsExampaper.id}" title = "修改"><i class="icon iconfont icon-caozuotubiao-xiugai"></i></a>
							<a href="${ctx}/exam/pbsExampaper/delete?id=${pbsExampaper.id} & sWay=1"
							onclick="return confirmx('确认要删除该试卷信息吗？', this.href)" title = "删除"><i class="icon iconfont icon-caozuotubiao-shanchu"></i></a></td>
					</shiro:hasPermission>
				</tr>
			</c:forEach>
		</tbody>
