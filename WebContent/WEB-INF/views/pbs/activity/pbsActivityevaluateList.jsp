		...
		<tbody>
			<c:forEach items="${page.list}" var="pbsActivityevaluate">
				<tr>
					...
					<shiro:hasPermission name="activity:pbsActivityevaluate:edit">
						<td><a
							href="${ctx}/activity/pbsActivityevaluate/form?id=${pbsActivityevaluate.id}" title = "修改"><i class="icon iconfont icon-caozuotubiao-xiugai"></i></a>
							<a
							href="${ctx}/activity/pbsActivityevaluate/delete?id=${pbsActivityevaluate.id}"
							onclick="return confirmx('确认要删除该活动评分吗？', this.href)" title = "删除"><i class="icon iconfont icon-caozuotubiao-shanchu"></i></a></td>
					</shiro:hasPermission>
				</tr>
			</c:forEach>
		</tbody>
