...
<script>
                ...
                if(data.length>0){
                    for(var i=0;i<data.length;i++){
                        ...
                        if (data[i]["type"]=="01"){
                            ...
                            // 生成新的文字插入对话框中
                            var html = '<tr><td style="text-align: left;"><a title="'+content+'" onclick="clickEventPreview(\'' + data[i]["objId"]
                                + '\')" class="ccmEventLayer' + data[i]["objId"] + ' '+readFlag +' active"  attrid="'
                                + data[i]["objId"] + '" attname="'
                                + data[i]["caseName"] + '" > ' + content + '</a></td></tr>';
                            content1 += html;
    ...
    function clickEventPreview(id) {
        ...
        layer.open({
            ...
            content: '${ctx}/preview/ccmEventIncidentPreview/formById?id='+id+ '&reportType=1'  //弹出层的url
        });
    }
