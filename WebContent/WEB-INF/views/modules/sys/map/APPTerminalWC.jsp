...
<!-- 警力样式 -->
<link rel="stylesheet"
	href="${ctxStatic}/flat/commandDispatch/css/reset.css" />
<link rel="stylesheet"
	href="${ctxStatic}/flat/commandDispatch/css/index.css" />
<link rel="stylesheet"
	href="${ctxStatic}/flat/commandDispatch/css/table.css" />
<script src="${ctxStatic}/jquery/jquery-1.8.3.min.js"></script>
<link rel="stylesheet"
	href="${ctxStatic}/DataTables/css/jquery.dataTables.css">
<script src="${ctxStatic}/DataTables/js/jquery.dataTables.js"
...
<script src="${ctxStatic}/jquery-jbox/2.3/jquery.jBox-2.3.min.js"
	type="text/javascript"></script>
<script src="${ctxStatic}/ol/ol-debug.js"></script>
...
<script src="${ctxStatic}/ccm/sys/js/APPTerminalWC.js"></script>
		...
		<div id="right" style="overflow: hidden;">
						...
						<div class="layui-tab layui-tab-brief" lay-filter="allTab">
							<ul class="layui-tab-title" id="GIStab">
								<li class="layui-this" type="event"><i
									class="nav-icon iconfont  icon-guijiguanli"></i>工作人员</li>
								<li type="event"><i
									class="nav-icon iconfont  icon-guijiguanli"></i>管控人员</li>
								...
								<div class="layui-tab-item layui-show">
									<div class="police-city hidden">
										<ul class="police-list" id="policeAll">
											<li style="width: 312px;">
												<div id="search" class="form-search"
													style="display: inline; width: 310px; padding: 10px 0 0 10px;">
													<label for="key" class="control-label"
														style="width: 15%; float: left; padding: 5px 5px 3px 0;">关键字:</label>
													<input type="text" class="empty" id="key" name="key"
														maxlength="50" style="width: 60%;" />
													<button class="btn btn-primary" id="btn"
														onclick="searchNode()" style="padding: 5px 0px 3px 0;">搜索</button>
												</div>
											</li>
											<li style="margin-top: 15px;">
												<div id="ztreePolice" class="ztree"
									...
									<div class="police-city">
										<ul class="police-list">
											<li style="width: 312px;">
												<div id="search" class="form-search"
													style="display: inline; width: 310px; padding: 10px 0 0 10px;">
													<label for="keyControl" class="control-label"
														style="width: 15%; float: left; padding: 5px 5px 3px 0;">关键字:</label>
													<input type="text" class="empty" id="keyControl"
														name="keyControl" maxlength="50" style="width: 60%;" />
													<button class="btn btn-primary" id="btnControl"
														onclick="searchNode()" style="padding: 5px 0px 3px 0;">搜索</button>
												</div>
											</li>
											<li style="margin-top: 10px;">
												<div id="ztreeControlPeople" class="ztree"
													style="margin-left: 10px;"></div>
											</li>
										</ul>
									</div>
      	...
      	<div class="checkbox checkbox-success radio-div tab-content-bg" style="display: inline-block;padding-top: 5px;padding-bottom: 5px;margin-left: 20px; margin-top: 10px;">
					...
					<label	for="realTime" style="padding:0;width:80px;" ><i class="nav-icon iconfont  icon-guijiguanli"></i>实时位置</label>
								...
								<div class="layui-tab-item">
									<ul class="ul-form">
										<li><input name="beginCreateDate" type="text"
											placeholder="请选择开始时间" id="beginCreateDate" maxlength="20"
											class="input-medium Wdate" value=""
											onclick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'endCreateDate\',{d:-7})}',maxDate:'#F{$dp.$D(\'endCreateDate\')||\'%y-%M\'}'});"
											style="height: 30px; margin-top: 10px;" /> <input
											name="endCreateDate" type="text" placeholder="请选择结束时间"
											id="endCreateDate" maxlength="20" class="input-medium Wdate"
											value=""
											onclick="WdatePicker({dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'beginCreateDate\',{d:7})||\'%y-%M\'}',minDate:'#F{$dp.$D(\'beginCreateDate\')}'})"
											style="height: 30px; margin-top: 10px;" /></li>
										<li>
											<div style="display: inline;width: 158px;">
												<input type="radio" id=primaryRadio name="drone" value="primaryRadio" checked ><span for="primaryRadio" >工作人员</span>
											</div>
											<div style="display: inline;width: 158px;margin-left: 10px;">
											 	<input type="radio" id="controlRadio" name="drone" value="controlRadio"><span for="controlRadio">管控人员</span>
											</div>
										</li>	
										<li id="liPrimary" style="margin-bottom: 10px">
											<sys:treeselect id="primaryPerson"
												name="primaryPerson.id" value="${office.primaryPerson.id}"
												labelName="office.primaryPerson.name" cssStyle="width:130px;margin:0"
											...
											<a href="javascript:;"  style="background: #f50b4b !important;" class="btn btn-primary ClearSubmit">
												<i class="icon-remove"></i> 清空
											</a>
										</li>
										<li id="liControl" style="display: none;"><sys:treeselect id="controlPeople" 
												name="" value="" labelName="office.primaryPerson.name" controlTree="true" 
												labelValue="${office.primaryPerson.name}" title="管控人员"
												url="/sys/map/treePeopleData" allowClear="true" isAll="true"
												notAllowSelectParent="true" />
										<a href="javascript:;"
											id="btnSubmitHis" onclick="HisTrackFun1()"
											class="btn btn-primary firstbtn"> <i class="icon-search"></i>
												查询
										</a> <a href="javascript:;"
											style="background: #f50b4b !important;"
											class="btn btn-primary ClearSubmit"> <i class="icon-remove"></i> 清空
										</a></li>
									</ul>
								</div>
							</div>
			...
			<div id="FullBody" style="position: relative;">
				...
				<div id="hideOrShowDiv"
					style="position: absolute; width: 100%; bottom: 0px; right: 3px; height: 0px; transition: height 1s; z-index: 1992">
					<!-- 收起按钮 -->
					<div
				...
				<div id="tool">
					<div id="toolCenter">
						<div class="row-fluid tool-container">
							<div class="span3 tool-list" id="ExportPng" style="width: 40%;">
								<i class="tool-icon tool-draw"></i><span>导出轨迹</span> </b>
							</div>
...
<script src="${ctxStatic}/common/div2img/canvas2png.js"
	type="text/javascript"></script>
<script>
    $(function(){
