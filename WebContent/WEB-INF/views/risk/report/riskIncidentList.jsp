...
<ul class="nav nav-tabs">
    ...
    <shiro:hasPermission name="report:riskIncident:edit">
        <li><a href="${ctx}/report/riskIncident/form">风险事件添加</a></li>
    </shiro:hasPermission>
</ul>
<form:form id="searchForm" modelAttribute="riskIncident" action="${ctx}/report/riskIncident/" method="post"
    ...
    <ul class="ul-form">
        ...
        <li><label>风险类型：</label>
            <form:select path="type" class="input-medium">
                ...
                <form:options items="${fns:getDictList('risk_risk_type')}" itemLabel="label" itemValue="value"
                              htmlEscape="false"/>
            </form:select>
        </li>
        <li><label>重要程度：</label>
    ...
    <c:forEach items="${page.list}" var="riskIncident">
        <tr>
            ...
            <c:if test="${riskIncident.importance eq '01'}">
                <td style='color:red'>${fns:getDictLabel(riskIncident.importance, 'ccm_conc_exte', '')}&nbsp; &nbsp;
                    <img src="${ctxStatic}/images/atteType_red.png"/></td>
            </c:if>
            <c:if test="${riskIncident.importance eq '02'}">
                <td style='color:orange'>${fns:getDictLabel(riskIncident.importance, 'ccm_conc_exte', '')}&nbsp; &nbsp;
            ...
            <c:if test="${riskIncident.urgency eq '01'}">
                <td style='color:red'>${fns:getDictLabel(riskIncident.urgency, 'ccm_conc_exte', '')}&nbsp; &nbsp; <img
                        src="${ctxStatic}/images/atteType_red.png"/></td>
            </c:if>
            <c:if test="${riskIncident.urgency eq '02'}">
                <td style='color:orange'>${fns:getDictLabel(riskIncident.urgency, 'ccm_conc_exte', '')}&nbsp; &nbsp;
                    <img src="${ctxStatic}/images/atteType_orange.png"/></td>
            </c:if>
            ...
            <td><shiro:hasPermission name="report:riskIncident:edit">
                <a class="btnList" href="${ctx}/report/riskIncident/form?id=${riskIncident.id}" title="修改"><i
                        class="iconfont icon-caozuotubiao-xiugai"></i></a>
                <a class="btnList" href="${ctx}/report/riskIncident/delete?id=${riskIncident.id}"
                   onclick="return confirmx('确认要删除该风险事件管理吗？', this.href)" title="删除"><i
                        class="iconfont icon-caozuotubiao-shanchu"></i></a>
            </shiro:hasPermission>
                <shiro:hasPermission name="log:ccmLogTail:view">
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/list?relevance_id=${riskIncident.id}&relevance_table=risk_incident', '记录信息', '800px', '660px')"
                       title="记录信息"><i class="iconfont icon-caozuotubiao-jiluxinxi" style="color: cornflowerblue;"></i></a>
                </shiro:hasPermission>
                <shiro:hasPermission name="log:ccmLogTail:edit">
                    <a class="btnList"
                       onclick="parent.LayerDialog('${ctx}/log/ccmLogTail/formProPermanent?relevance_id=${riskIncident.id}&relevance_table=risk_incident', '添加记录', '800px', '660px')"
